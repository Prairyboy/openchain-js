...
ApiClient.prototype.httpGet = function (url) {
    debug('httpGet: url', url);
    return fetch(url)
        .then(function(response) {
            debug('httpGet -> fetch: response', _.pick(response, ['url', 'status']))
...
function parseResponse(response) {
    if (response.status !== 200) {
        debug('parseResponse (response.status !== 200): response.status', chalk.red(response.status))
        return response.json().then( err => {
            throw err;
        });
    } else {
        debug('parseResponse (response.status === 200): response', _.pick(response, ['url', 'status']));
...
//pp
ApiClient.prototype.getRecordsByName = function (name, type) {
    var getRecordsByName = this.httpGet(this.endpoint + "query/recordsbyname?name=" + encodeURIComponent(name.toString()) + "&type=" + encodeURIComponent(type.toString()));
    return getRecordsByName
        .then(function (result) {
            var records = [];
            for (var i = 0; i < result.length; i++) {
                let item = parseRecord(result[i]);
                if (item.value.remaining() === 0) {
                    item.data = null;
                }
                else {
                    item.key = encoding.decodeString(item.key);
                    item.data = encoding.decodeString(item.value);
                }
                records.push(item);
            }
            return records;
...
//pp
ApiClient.prototype.getTransactionsByPath = function (path) {
    return this.httpGet(this.endpoint + "query/GetTransactionsByPath?path=" + encodeURIComponent(path.toString()));
};

ApiClient.prototype.getFilteredTransactions = function (path, filter) {
    return this.httpPost(
        this.endpoint + "query/GetFilteredTransactions",
        JSON.stringify({ path: path, filter: filter }));
};
