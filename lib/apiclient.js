...
ApiClient.prototype.httpGet = function (url) {
    ...
    return fetch(url)
        .then(function(response) {
            debug('httpGet -> fetch: response', response)
...
function parseResponse(result) {
    if (result.status !== 200) {
        debug('parseResponse (response.status !== 200): result.status', result.status)
        throw result;
    } else {
        debug('parseResponse (response.status === 200): result', result);
        return result.json();
    };
};
